...
def flat(mask):
    ...
    h = 28                      #shape of cross transpose attention map is 28 x 28
    mask = F.interpolate(mask,size=(int(h),int(h)), mode='bilinear') #downsample
    x = mask.view(batch_size, 1, -1).permute(0, 2, 1) #Fattening
    g = x @ x.transpose(-2,-1) #Self Product
    gt_att = g.unsqueeze(1) 
    return gt_att
...
def parser():
    ...
    parser.add_argument('--savepath', default="../src/pretrained-models", type=str)
    parser.add_argument('--datapath', default="../src/data/Kvasir-SEG/train", type=str)
    parser.parse_args()
...
def train(Dataset, Network):
    ...
    for epoch in range(cfg.epoch):
        ...
        for step, (image, mask) in enumerate(loader):
            ...
            att_loss = att_loss(attmap,mask,wr,ws) # attention loss
            loss1 = bce_iou_loss(p1,mask) #iou loss
            loss2 = bce_iou_loss(wr,mask)*0.125+bce_iou_loss(ws,mask)*0.125 # auxillary_loss
            loss = loss1+loss2+att_loss
        ...
        if epoch >= 100 and args.local_rank==0:
